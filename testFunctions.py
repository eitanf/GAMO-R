...
ROSf = TestFn("Rosenbrock's Saddle", lambda X: ((1-X[0])**2)+100*((X[1]-X[0]**2)**2), dimension=2)
...
PARABOLA = TestFn("Parabola", lambda X: sum([x_i**2 for x_i in X]), dimension=3)

# Step function in 5 dimensions
STEP = TestFn("Step function", lambda X: sum([math.floor(X[i]) for i in range(len(X))]) + 6*len(X), dimension=5)

# Quartic with noise in 30 dimensions
NOISY_QUARTIC = TestFn("Quartic with noise", lambda X: sum([i*(X[i]**4) for i in range(len(X))]) + random.uniform(0,1), dimension=30)
...
SHEKEL = TestFn("Shekel's Foxholes", shekel, dimension=2)
